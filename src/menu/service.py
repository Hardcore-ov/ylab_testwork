...
# async def create_menu(menu_in: MenuCreate, session: AsyncSession) -> MenuOut:
#     await validated_menu.validate_title(menu_in.title, session)
#     menu = service.create(menu_in, session)
...
class MenuCache:
    ...
    async def get_menu(self, menu_id):
        ...
        if cached:
            return cached
        await validated_menu.check_exists(menu_id, self.session)
